...
interface Results {
  ...
  qualitative: { code: string; text: string; textAm?: string; responses: { responseId: number; evaluatorLevel: string; completedAt: string; text: string }[] }[];
}
const versions = [
...
export default function ResultsDashboard({ surveyId, surveyName }: { surveyId: string; surveyName: string }) {
  ...
  return <div className="results-dashboard">
    ...
    {results && !pending && <>
      ...
      <Panel id="qualitative-feedback" title="Open-ended feedback" note="Read responses in context. Similar wording may be grouped during qualitative analysis, but the system does not automatically infer themes or sentiment.">
        <div className="qualitative-results">{results.qualitative.map(question => <article key={question.code}>
          <header><code>{question.code}</code><div><h3>{question.text}</h3>{question.textAm && <p lang="am">{question.textAm}</p>}</div><span>{question.responses.length} responses</span></header>
          {question.responses.length ? <ul>{question.responses.map(response => <li key={`${question.code}-${response.responseId}`}><p>{response.text}</p><small>Response #{response.responseId} · {role(response.evaluatorLevel)} · {date(response.completedAt)}</small></li>)}</ul> : <p className="empty-state">No written responses for this question.</p>}
        </article>)}</div>
      </Panel>
